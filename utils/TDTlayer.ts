...
export default class TDTlayer extends WebTileLayer {
  ...
  getTileUrl(level: number, row: number, col: number) {
      return "http://t" + row % 8 + ".tianditu.gov.cn/img_c/wmts?SERVICE=WMTS&REQUEST=GetTile&VERSION=1.0.0&LAYER=img&STYLE=default&TILEMATRIXSET=c&TILEMATRIX=" + level + "&TILEROW=" + row + "&TILECOL=" + col + "&FORMAT=tiles&tk=f6e0f7525c9d5618f59bb0b9cde93751";
   }
